...
const CalendarList = (props: CalendarListProps & ContextProp, ref: any) => {
	...
	const shouldFixRTL = useMemo(
		() => !constants.isRN73() && constants.isAndroidRTL && horizontal,
		[horizontal],
	);
	/**
	...
	const items: CustomDate[] = useMemo(() => {
		...
		for (let i = 0; i <= pastScrollRange + futureScrollRange; i++) {
			const rangeDate = addMonthsToDate(
				initialDate.current,
				i - pastScrollRange,
			);
	...
	const initialDateIndex = useMemo(() => {
		return findIndex(
			items,
			(item) => item.toString() === initialDate.current?.toString(),
		);
	}, [items]);
	...
	const scrollToDay = (
		...
		const diffMonths = Math.round(
			getDiffInMonths(
				setDayOfMonth(initialDate?.current, 1),
				setDayOfMonth(scrollTo, 1),
			),
		);
		let scrollAmount =
	...
	const getItemLayout = useCallback(
		(_: ArrayLike<CustomDate> | undefined | null, index: number) => {
			return {
				length: calendarSize,
	...
	const renderItem = useCallback(
		({ item }: { item: CustomDate }) => {
			...
			const onHeaderLayoutToPass = shouldMeasureHeader.current
				? onHeaderLayout
				: undefined;
	...
	return (
		<View style={style.current.flatListContainer} testID={testID}>
			<FlatList
				ref={list}
				windowSize={
					shouldFixRTL ? pastScrollRange + futureScrollRange + 1 : undefined
				}
				style={listStyle}
