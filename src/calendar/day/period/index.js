...
const PeriodDay = props => {
  const {theme, marking, date, onPress, onLongPress, state, accessibilityLabel, testID, children, limiteDate} = props;
  ...
  const isBeforeLimitDate = (dateString, limitDateString) => {
    if (!dateString) {
      return false;
    }
    const givenDate = new Date(dateString);
    const limitDate = new Date(limitDateString || '2023-01-01'); // par défaut à '2023-01-01' si non fourni
