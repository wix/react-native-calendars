...
const Calendar = (props: CalendarProps & ContextProp) => {
	...
	useDidUpdate(() => {
		...
		AccessibilityInfo.announceForAccessibility(
			formatDate(_currentMonth, "MMMM yyyy"),
		);
	}, [currentMonth]);
	...
	const handleDayInteraction = useCallback(
		(date: DateData, interaction?: (date: DateData) => void) => {
			...
			if (
				allowSelectionOutOfRange ||
				(!(minDate && !isGTE(day, getDate(minDate))) &&
					!(maxDate && !isLTE(day, getDate(maxDate))))
			) {
	...
	const shouldDisplayIndicator = useMemo(() => {
		if (currentMonth) {
			const lastMonthOfDay = toMarkingFormat(
				subtractDaysToDate(setDayOfMonth(addMonthsToDate(currentMonth, 1), 1), 1),
			);
