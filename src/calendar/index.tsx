...
const Calendar = (props: CalendarProps) => {
  ...
  const WeekSelectionComponent = ({infos, checked}: {infos: SelectedWeek; checked: boolean}) => {
    return (
      <TouchableOpacity
        onPress={() => {
          ...
          props.onWeekSelected?.(infos, selectedWeeks);
        }}
      >
        <View
          style={[style.current.weekSelectionComponent, checked && style.current.weekSelectionComponentChecked]}
        ></View>
      </TouchableOpacity>
    );
  };
  ...
  const renderWeek = (days: XDate[], id: number) => {
    ...
    if (props.showWeekNumbers) {
      week.unshift(renderWeekNumber(weekNumber));
    }

    return (
      // <View style={style.current.week} key={id}>
      <View style={style.current.week} key={id}>
        {props.customWeekSelectionComponent?.({weekNumber, firstDate: days[0]}) ||
          (!!props.onWeekSelected && (
