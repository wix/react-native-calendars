...
const CalendarHeader = forwardRef((props, ref) => {
  ...
  //
  const isExactlyOnLimitDate = useMemo(() => {
    if (!month || !limiteDate) return false;
    const limitDate = new XDate(limiteDate);
    return month.diffMonths(limitDate) === 0;
  }, [month, limiteDate]);
  ...
  const _renderArrow = direction => {
    ...
    const shouldDisable = direction === 'left' ? disableArrowLeft || isExactlyOnLimitDate : disableArrowRight;
