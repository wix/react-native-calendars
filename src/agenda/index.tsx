...
import CalendarList, {
	type CalendarListImperativeMethods,
...
import ReservationList, { type ReservationListProps } from "./reservation-list";
...
export default class Agenda extends Component<AgendaProps, State> {
	...
	loadReservations(props: AgendaProps) {
		if (
			...
			this.setState({ firstReservationLoad: true }, () => {
				this.props.loadItemsForMonth?.(dateToData(this.state.selectedDay));
			});
		}
	}
	...
	chooseDay(d: DateData, optimisticScroll: boolean) {
		const day = getDate(d.dateString);
		...
		this.props.loadItemsForMonth?.(dateToData(day));
		this.props.onDayPress?.(dateToData(day));
	}
	...
	onDayChange = (day: CustomDate) => {
		...
		this.props.onDayChange?.(dateToData(day));
	};
