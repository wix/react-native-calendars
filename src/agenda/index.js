...
export default class AgendaView extends Component {
  ...
  addMonth = count => {
    this.setState({ calendarMonthChanged: true })
    this.updateMonth(this.state.calendarCurrentMonth.clone().addMonths(count, true))
  }

  updateMonth = (day, doNotTriggerListeners) => {
    if (day.toString("yyyy MM") === this.state.calendarCurrentMonth.toString("yyyy MM")) {
      return
    }
    this.setState(
      {
        calendarCurrentMonth: day.clone(),
      },
      () => {
        if (!doNotTriggerListeners) {
          const currMont = this.state.calendarCurrentMonth.clone()
  ...
  render() {
    ...
    const { agendaOpen, selectedDay, calendarCurrentMonth, calendarMonthChanged } = this.state

    return (
      <View onLayout={this.onLayout} style={[this.props.style, { flex: 1, overflow: "hidden" }]}>
        <View style={this.styles.reservations}>{agendaOpen && this.renderReservations()}</View>
        <Animated.View style={headerStyle}>
          <Animated.View style={{ flex: 1, transform: [{ translateY: contentTranslate }] }}>
            ...
            <View style={{ opacity: !agendaOpen ? 0 : 1 }}>
              <CalendarList
                onLayout={() => {
