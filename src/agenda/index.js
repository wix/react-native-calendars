...
export default class AgendaView extends Component {
  ...
  loadReservations(props) {
    if ((!props.items || !Object.keys(props.items).length) && !this.state.firstResevationLoad) {
      this.setState({ firstResevationLoad: true }, () => {
  ...
  componentDidUpdate(props) {
    let pPropsItem = props.items;
    let nPropsItem = this.props.items;
    let isSameItems = JSON.stringify(pPropsItem) === JSON.stringify(nPropsItem);
    if (!isSameItems) {
      this.setState({ firstResevationLoad: false });
    } else {
      this.loadReservations(this.props);
    }
  }
  ...
  render() {
    ...
    return (
      <View onLayout={this.onLayout} style={[this.props.style, { flex: 1, overflow: 'hidden' }]}>
        <NavigationEvents
          onWillFocus={payload => {
            console.log("Agenda", "will focus", payload);
            this.componentToBeMount()
          }}
          // onDidFocus={payload => {
          //   console.log("Agenda", "did focus", payload);
          // }}
          onWillBlur={payload => {
            console.log("Agenda", "will blur", payload);
            this.componentToBeUnmount();
          }}
        // onDidBlur={payload => {
        //   console.log("Agenda", "did blur", payload);
        // }}
        />
