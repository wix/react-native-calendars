...
export default class AgendaView extends Component {
  ...
  onScrollPadLayout() {
    ...
    // set the initial position of the touchable portion of the knob
    this.setScrollPadPosition(
      this.props.calendarView ? 0 : this.initialScrollPadPosition(),
      false
    );
  ...
  addMonth = count => {
    ...
    this.updateMonth(
      this.state.calendarCurrentMonth.clone().addMonths(count, true)
    );
  };
  ...
  updateMonth = (day, doNotTriggerListeners) => {
    if (
      ...
      this.state.calendarCurrentMonth.toString("yyyy MM")
    ) {
      return;
    }
    this.setState(
    ...
    return (
      ...
      >
        <View style={this.styles.reservations}>
          {agendaOpen && this.renderReservations()}
        </View>
