...
import type { DateData, MarkedDates } from "../../types";
...
const WeekCalendar = (props: WeekCalendarProps) => {
	...
	const items = useRef<string[]>(
		getDatesArray(current ?? date, firstDay, numberOfDays),
	);
	...
	const shouldFixRTL = useMemo(
		() => !constants.isRN73() && constants.isAndroidRTL,
		[],
	);
	...
	useDidUpdate(() => {
		if (updateSource !== UpdateSources.WEEK_SCROLL) {
			...
			if (pageIndex !== currentIndex.current) {
				const adjustedIndexFrScroll = shouldFixRTL
...
function getDateForDayRange(
	...
	if (weekIndex !== 0) {
		d = addDaysToDate(d, numberOfDays * weekIndex);
	}
...
function getDate(
	...
	if (weekIndex !== 0) {
		d = addDaysToDate(d, firstDay - dayOfTheWeek);
	}
	const newDate =
		numberOfDays && numberOfDays > 1
			? addDaysToDate(d, weekIndex * numberOfDays)
			: addWeeksToDate(d, weekIndex);
	const today = getCurrentDate();
	const offsetFromNow = getDiffInDays(newDate, today);
