	...
	describe("WeekCalendar updates", () => {
		...
		describe("arrow press", () => {
			...
			it("should call onMonthChange when new week first day is in a different month", () => {
				...
				const diff =
					Math.ceil(
						((getUTCDayOfMonth(endOfMonth) + 1) - (getUTCDayOfMonth(today)) / 7),
					) + (getUTCDayOfWeek(today) > getUTCDayOfWeek(endOfMonth) ? 1 : 0);
				const expectedDate = setDayOfMonth(
					today,
					getDay(today) + 7 * diff - getDayOfWeek(today),
				);
