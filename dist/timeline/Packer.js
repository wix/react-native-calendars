...
function buildEvent(event, left, width, { dayStart = 0, hourBlockHeight = HOUR_BLOCK_HEIGHT }) {
    ...
    return Object.assign(Object.assign({}, event), { top: (dayStartTime.diffHours(startTime) - dayStart) * hourBlockHeight, height: startTime.diffHours(endTime) * hourBlockHeight, width,
        left });
}
...
export function populateEvents(_events, populateOptions) {
    ...
    const events = _events
        .map((ev, index) => (Object.assign(Object.assign({}, ev), { index: index })))
        .sort(function (a, b) {
        if (a.start < b.start)
            return -1;
        if (a.start > b.start)
